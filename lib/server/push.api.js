...
//修改这里的gcm为jpush.名字不变,要不然需要改很多rocket.chat的代码.
    if (options.gcm && options.gcm.apiKey) {
        ...
        self.sendGCM = function(userTokens, notification) {
            ...
            var jpush = Npm.require('../lib/JPush/JPush.js');
            ...
            var jpush_message = jpush.android(data.message,data.title,null,data.msgcnt,0,null,data.style);

            if (Push.debug) {
              console.log('Create GCM Sender using "' + options.gcm.apiKey + '"');
            }
            var sender = new gcm.Sender(options.gcm.apiKey);
            var client = jpush.buildCient(options.gcm.apiKey,options.gcm.projectNumber);
            ...
            client.push().setPlatform('android')
              .setAudience(jpush.registration_id(userToken))
              .setNotification(jpush_message)
              .setOptions(null,60)
